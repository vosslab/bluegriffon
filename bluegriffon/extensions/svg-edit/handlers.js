...
$(function() {
  ...
  function moz_file_picker(readflag, callback) {
      var fp = window.Components.classes["@mozilla.org/filepicker;1"].
    createInstance(Components.interfaces.nsIFilePicker);
      if(readflag)
    fp.init(window.browsingContext, "Pick a SVG file", fp.modeOpen);
      else
    fp.init(window.browsingContext, "Pick a SVG file", fp.modeSave);
  ...
  svgCanvas.setCustomHandlers({
    'open':function() {
      ...
      moz_file_picker(true, function(file) {
        try {
          if(!file)
              return;
    ...
    'save':function(svg, str) {
      try {
        moz_file_picker(false, function(file) {
          try {
            if(!file)
              return;
